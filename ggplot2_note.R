...
#矢量图是过程化的，意味着图形可无限缩放而没有细节的损失
#光栅图以像素阵列形式存储，具有固定的最优观测大小

#一般来说，矢量输出更受人喜爱，但对于包含数千个对象的图形，矢量渲染会变得很慢，此时最好转化为光栅输出。
#对于图形印刷，高分辨率可能是较好的折中方案，但是图形可能会很大

#ggsave()是为了图形交互而优化过的函数
#1.path设置图形存储的路径，它会根据文件扩展名自动选择正确的图形设备

#2.三个控制输出尺寸的参数。若空白，则使用当前屏幕图形设备尺寸
#width和height设置绝对尺寸大小，scale设置图形相对屏幕展示的尺寸大小
#当最终图形绘制好后，最好设置width和width，这样可以精确控制输出尺寸

#3.对于光栅图形，dpi参数控制图形的分辨率，默认值为300，适合大部分打印设备

qplot(mpg,wt,data = mtcars)
ggsave(filename = "outfile.pdf")
pdf(file = "outfile.pdf",width = 6,height = 6)
qplot(mpg,wt,data = mtcars)
